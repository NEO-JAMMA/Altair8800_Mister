...
#
# Cyril Venditti - Altera mif format added
...
if len(sys.argv) != 4:
    ...
    print("       output_format: mem, coe or mif")
    sys.exit()
...
if output_format == 'mem':
    ...
    image_output += mif_header
    image_output += str(width*height) + ";"
    image_output += mif_header_2
    image_output += str(colour_bits) + ";"
    image_output += mif_header_3
    index = 0
    for d in image_data:
        image_output += hex(index)[2:] + ":\t" + hex(d)[2:] + ";\n"
        index+= 1
    image_output += "END;"
else:
    print("Error: output_format should be mem, coe or mif.")
    sys.exit()
...
if output_format == 'mem':
    ...
    palette_output += mif_header
    palette_output += str(pal_size) + ";"
    palette_output += mif_header_2
    palette_output += str(12) + ";"
    palette_output += mif_header_3
    for i in range(pal_size):
	    pal_entry = colours[i][0] * 256 + colours[i][1] * 16 + colours[i][2]
	    palette_output += hex(i)[2:] + ":\t"+hex(pal_entry)[2:] + ";\n"
    palette_output += "END;"
else:
    print("Error: output_format should be mem, coe or mif.")
    sys.exit()
